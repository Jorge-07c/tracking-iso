...
[data-testid="stMetric"]:hover {
    border-top-color: var(--teal) !important; box-shadow: 0 4px 20px #05966918 !important;
}
...
# ════════════════════════════════════
COLORES_ESTADO = {
    "Recibido": "#6b7280", "En Proceso": "#1d4ed8",
    "Control Calidad": "#7c3aed", "Terminación": "#059669", "Despacho": "#d97706"
}
...
def tarjeta_orden(serial, cliente, tipo, estado, dias):
    ...
    return f"""
    <div style="background:#ffffff;border:1px solid #d1d5db;border-left:3px solid {c};
...
def header_modulo(icono, titulo, subtitulo):
    ...
    <hr style="border-color:#d1d5db;margin:0 0 20px 0;">
    """, unsafe_allow_html=True)
...
def checklist_visual(orden):
    ...
    for i, etapa in enumerate(ESTADOS_L):
        ...
        elif i == idx: bg, col, dot = "#1d4ed822", "#1d4ed8", ICONOS_L[etapa]
        else:          bg, col, dot = "#1a1e2a",   "#3a3f50", "○"
        conector = ""
        if i < len(ESTADOS_L) - 1:
            lc = "#059669" if i < idx else "#2a2f3e"
...
def historial_html(historial):
    ...
    for h in reversed(historial):
        quien = f" · <span style='color:#1d4ed8;'>{h['actualizado_por']}</span>" if h.get("actualizado_por") else ""
        items += f"""<div style="border-left:2px solid #e5e7eb;padding:5px 12px;margin:4px 0;
                        font-family:'Share Tech Mono',monospace;font-size:0.72rem;">
            <span style="color:#1d4ed8;">{ICONOS_MAP.get(h.get('estado',''),'●')}</span>
    ...
    with col:
        ...
        <div style="text-align:center;margin-bottom:28px;">
            <div style="font-family:'Rajdhani',sans-serif;font-size:3rem;font-weight:700;
                        ...
                        text-shadow:0 0 30px #1d4ed855;">⚙ TRACKISO</div>
        ...
        st.markdown("""<div style="background:#ffffff;border:1px solid #d1d5db;border-top:2px solid #1d4ed8;
...
ROL_C = {"Administrador": "#1d4ed8", "Ventas": "#059669", "Producción": "#7c3aed"}
...
with st.sidebar:
    ...
    <div style="padding:8px 0 14px;text-align:center;">
        <div style="font-family:'Rajdhani',sans-serif;font-size:1.5rem;font-weight:700;
                    color:#1d4ed8;letter-spacing:3px;">⚙ TRACKISO</div>
        <div style="font-family:'Share Tech Mono',monospace;color:#9ca3af;font-size:0.62rem;letter-spacing:2px;">
            ISO 9001:2015</div>
    </div>
    <hr style="border-color:#e5e7eb;margin:0 0 14px;">
    <div style="background:#ffffff;border:1px solid #d1d5db;border-left:3px solid {rc};
        ...
        <div style="color:#111827;font-weight:600;font-size:0.9rem;">👤 {st.session_state.usuario}</div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:5px;margin-bottom:14px;">
        <div style="background:#ffffff;border:1px solid #d1d5db;border-top:2px solid #1d4ed8;
            ...
            <div style="font-family:'Rajdhani',sans-serif;font-size:1.35rem;font-weight:700;
                        color:#111827;">{len(_todas)}</div>
            <div style="color:#6b7280;font-size:0.58rem;font-family:'Share Tech Mono',monospace;
                        text-transform:uppercase;">Total</div>
        </div>
        <div style="background:#ffffff;border:1px solid #d1d5db;border-top:2px solid #059669;
                    border-radius:4px;padding:9px 6px;text-align:center;">
            <div style="font-family:'Rajdhani',sans-serif;font-size:1.35rem;font-weight:700;
                        color:#111827;">{_activas}</div>
            <div style="color:#6b7280;font-size:0.58rem;font-family:'Share Tech Mono',monospace;
                        text-transform:uppercase;">Activas</div>
        </div>
        <div style="background:#ffffff;border:1px solid #d1d5db;border-top:2px solid #d97706;
                    border-radius:4px;padding:9px 6px;text-align:center;">
            <div style="font-family:'Rajdhani',sans-serif;font-size:1.35rem;font-weight:700;
                        color:#111827;">{_despachadas}</div>
            <div style="color:#6b7280;font-size:0.58rem;font-family:'Share Tech Mono',monospace;
...
if menu == "📋 Cotización":
    ...
    st.markdown(f"""
    <div style="background:#ffffff;border:1px solid #d1d5db;border-radius:6px;
        ...
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;">
            <div>
                ...
                <div style="color:#111827;font-weight:600;overflow:hidden;text-overflow:ellipsis;
                            white-space:nowrap;">{cliente or "—"}</div>
            </div>
            <div>
                <div style="color:#6b7280;font-size:0.62rem;font-family:'Share Tech Mono',monospace;
                            text-transform:uppercase;letter-spacing:1px;margin-bottom:4px;">Cantidad</div>
                <div style="color:#1d4ed8;font-family:'Rajdhani',sans-serif;font-weight:700;
            ...
            <div>
                ...
                <div style="color:#059669;font-family:'Rajdhani',sans-serif;font-weight:700;
    ...
    for orden in lista:
        ...
        if isinstance(dias, int) and dias > orden["tiempo"]:
            st.markdown(f"""<div style="background:#dc262615;border:1px solid #dc262640;border-left:3px solid #dc2626;
                ...
                font-family:'Share Tech Mono',monospace;font-size:0.75rem;color:#dc2626;">
        ...
        if nuevo_estado == "Despacho" and orden["estado"] != "Despacho":
            st.markdown("""<div style="background:#d9770615;border:1px solid #d9770640;
                ...
                font-size:0.78rem;color:#d97706;font-family:'Share Tech Mono',monospace;">
        ...
        if orden.get("specs"):
            with st.expander("📋 Especificaciones técnicas"):
                st.markdown(f'<span style="color:#4b5563;font-size:0.88rem;">{orden["specs"]}</span>', unsafe_allow_html=True)
...
# ════════════════════════════════════
    ...
    with col_b:
        ...
        for label, valor, color in [("💰 Total en órdenes", total_val, "#e8eaf0"),
            ...
            st.markdown(f"""
            <div style="background:#ffffff;border:1px solid #d1d5db;border-left:3px solid {color};
    ...
    cp = {"Recibido":"#6b7280","En Proceso":"#1d4ed8","Control Calidad":"#7c3aed","Terminación":"#059669","Despacho":"#d97706"}
    for estado in ESTADOS:
        ...
        st.markdown(f"""<div style="margin-bottom:7px;">
            <div style="display:flex;justify-content:space-between;margin-bottom:4px;">
                <span style="color:#4b5563;font-size:0.78rem;">{ICONOS[estado]} {estado}</span>
...
# ════════════════════════════════════
    ...
    if buscar:
        ...
        if not resultados:
            ...
            for orden in resultados:
                c = COLORES_ESTADO.get(orden["estado"], "#6b7280")
                st.markdown(f"""<div style="background:#ffffff;border:1px solid #d1d5db;border-left:3px solid {c};
                    ...
                    <span style="color:#111827;font-weight:600;">{orden['cliente']}</span>
                    <span style="color:#9ca3af;margin:0 8px;">·</span>
                    <span style="color:#4b5563;font-size:0.85rem;">{orden['tipo']}</span>
                </div>""", unsafe_allow_html=True)
                checklist_visual(orden)
                historial_html(orden["historial"])
    ...
    if not eventos:
        ...
        lineas = ""
        cp2 = {"Recibido":"#6b7280","En Proceso":"#1d4ed8","Control Calidad":"#7c3aed","Terminación":"#059669","Despacho":"#d97706"}
        for ev in eventos[:40]:
            fecha_ev = (ev.get("fecha") or "—")[:16].replace("T", " ")
            quien    = f" <span style='color:#1d4ed8;'>{ev['actualizado_por']}</span>" if ev.get("actualizado_por") else ""
        ...
        st.markdown(f'<div style="background:#f8f9fb;border:1px solid #e5e7eb;border-radius:6px;padding:14px 18px;">{lineas}</div>', unsafe_allow_html=True)